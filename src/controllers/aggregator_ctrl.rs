...
pub async fn aggregator(start: String, end: String, agg_type: String, group_id: i64, parent_id_pre: String) -> Result<Json<UserData>, ErrorMessage> {
    if ! dates::is_valid_date(&start) || ! dates::is_valid_date(&end) {
        println!("Invalid date range {} to {}", start, end);
        return Err(ErrorMessage::BadRequest(Status::BadRequest, "Invalid date".to_string()));
    }

    let rows: Vec<DailyRowsCount> = daily_rows_count::select_by_date(start.clone(), end.clone(), group_id.clone(), agg_type.clone(), parent_id_pre.clone()).await;
    Ok(Json(UserData {
        agg_type,
        start,
        end,
...
fn aggregate(rows: Vec<DailyRowsCount>) -> HashMap<String, HashMap<String, HashMap<String, ProcessData>>> {
